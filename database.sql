...
quantity int);
select * from cart;
create table orders(orderid int auto_increment primary key,orderdate date ,cartid int,
itemname varchar(50),quantity int,price int,costprice int);
select * from orders;
insert into admin(aname,password) values('Likki','Likki@123');
show tables;
commit;
alter table orders add constraint fk_orders_cart foreign key(cartid) 
references cart(cartid) on delete set null;
